...
class Application(tk.Frame):
	...
	def createTempMenu(self):
		...
		for child in level: 
			...
			settingValue = Label(self.menuFrame, text=child.value[0], font=('Helvetica', 36))
			settingValue.grid(row=rowNumber, column=1)
			settingKey = Button(self.menuFrame, text=child.name, font=('Helvetica', 36), command=lambda: self.changeMenuValue(child, settingValue))
